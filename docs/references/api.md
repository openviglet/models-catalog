...
| `…/query-manifest.json` | **Structured-RAG field manifest** — a per-field descriptor over the flattened record shape (`catalog.ndjson`): `type`, `facet`, `multiValued`, `enum` value sets, numeric `min`/`max`, `sortable` and a human `description`. The contract an external vectorless / structured-data RAG declares its field schema from and constrains NL→filter against. Its own envelope. |
| `…/context.txt` | **LLM context bundle** — a compact, token-budgeted digest (one decision line per model) for the "stuff the whole catalog" vectorless mode and as an assistant-ingestible GEO artifact (complements `llms.txt`). Plain text. |
| `…/qa-eval.jsonl` | **Grounded-answer eval set** — a curated `question → expected model id(s) / structured filter` list (JSON Lines). Seeds the "Ask the catalog" widget's example prompts and doubles as a drift check (filters reference `query-manifest.json` fields). Present only when `catalog/qa-eval.jsonl` exists. |
| `…/endpoints.json` | **Discovery manifest** — a machine-readable map of every published path (absolute URLs): `latest`, `pinned`, `index`, `stats`, `coverage`, `leaderboards`, `changes`, `feed`, `csv`, `ndjson`, `queryManifest`, `context`, `qaEval` (when present), `aliases`, `schema`, `plans` / `providers` (+ their `*Schema` when present), and the available `byKind` / `byVendor` / `byCapability` / `byModality` slice keys. Read this to discover the surface rather than hard-coding paths. |
